...
void ImporterModel::checkInputPath(int row)
{
	...
	if (entry.path.isEmpty()) {
		...
		const char *program =
			detect_program(entry.path.toStdString().c_str());
		entry.program = QTStr(program);

		if (strcmp(program, "") == 0) {
			entry.selected = false;
		} else {
			char *name = get_sc_name(
				entry.path.toStdString().c_str(), program);
			entry.name = name;
	  ...
	  ui(new Ui::OBSImporter)
{
	...
	if (!autoSearchPrompt) {
		QMessageBox::StandardButton button = OBSMessageBox::question(
			this, QTStr("Importer.AutomaticCollectionPrompt"),
	...
	if (autoSearch)
		f = importers_find_files();
	else
		da_init(f);

	for (size_t i = 0; i < f.num; i++) {
		QString path = f.array[i];
		path.replace("\\", "/");
	...
	QModelIndex index =
		optionsModel->createIndex(optionsModel->rowCount() - 1, 1);
	QMetaObject::invokeMethod(ui->tableView, "setCurrentIndex",
				  Qt::QueuedConnection,
				  Q_ARG(const QModelIndex &, index));
}

OBSImporter::~OBSImporter()
{
	importers_free();
}
...
void OBSImporter::importCollections()
{
	...
	for (int i = 0; i < optionsModel->rowCount() - 1; i++) {
		...
		import_sc(path.toStdString().c_str(),
			  name.toStdString().c_str(), &res);

		if (res != NULL) {
			const char *name = obs_data_get_string(res, "name");
			QString file = name;
			...
			obs_data_save_json(res, save.toStdString().c_str());
